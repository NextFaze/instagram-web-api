...
class Instagram {
  ...
  async getPhotosByUsername({ username, first, after }) {
    try {
      ...
      if(!user) {
        throw new Error('Could not retrieve user with username: ',  username);
      }
      return this.getUserIdPhotos({ id: user.id, first, after })
    }
    catch(err) {
      console.log("[getPhotosByUsername] could not get photos for user: ", username, JSON.stringify(err));
    }
  }
